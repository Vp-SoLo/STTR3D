...
from utilities.integration_tools import batched_index_select, torch_1d_sample, NestedTensor, Arguments
...
class RegressionHead3D(nn.Module):
    ...
    """
    def __init__(self, args: Arguments, cal=None, ot: bool = False):
        super(RegressionHead3D, self).__init__()
        self.dustbin = nn.Parameter(torch.ones(1))
        self.threshold = 0.5
        self.args = args
        self.cal = cal
        self.ot = ot

    def _sinkhorn(self, Z: Tensor, log_mu: Tensor, log_nu: Tensor, iters: int):
    ...
    def _optimal_transport(self, scores: Tensor, iters: int):
        """ Perform Differentiable Optimal Transport in Log-space for stability"""
        b, m, n = scores.shape
        one = scores.new_tensor(1)
        ms, ns = (m * one).to(scores), (n * one).to(scores)

        bins0 = self.dustbin.expand(b, m, 1)
        bins1 = self.dustbin.expand(b, 1, n)
        alpha = self.dustbin.expand(b, 1, 1)
        ...
        Z = self._sinkhorn(couplings, log_mu, log_nu, iters)
        Z = Z - norm  # multiply probabilities by M+N
        return Z

    def forward(self, attn: Tensor, disp_t1: Tensor, disp_t2: Tensor, feat_t1: Tensor, feat_t2: Tensor):
        bs, h, w = disp_t1.shape

        if self.ot:
            attn = self._optimal_transport(attn, self.args.ot_iter)

        max_ = torch.max(attn[...,:-1,:-1], dim=2)
        idc = max_.indices

        # flatten disp
        disp_t1 = torch.reshape(disp_t1, (bs, h*w))
        disp_t2 = torch.reshape(disp_t2, (bs, h*w))

        disp_t2 = torch.take_along_dim(disp_t2, idc)
        # forward disparity change
        disp_change = disp_t2 - disp_t1
        disp_change = torch.reshape(disp_change, (bs, h, w))

        # adjust
        disp_change = self.cal(disp_change, feat_t1, feat_t2)
