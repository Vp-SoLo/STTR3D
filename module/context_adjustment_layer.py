...
def build_context_adjustment_layer(args: Arguments):
...
class ContextAdjustmentLayer3D(nn.Module):
    """
    Adjust the disparity change based on image context
    """

    def __init__(self, num_res: int, feature_dim: int = 3, feature_res: int = 16, expansion_ratio: int = 3,
                 downsample: int = 4, args: Arguments = None):
        super(ContextAdjustmentLayer3D, self).__init__()
        self.args = args
        self.upsample = downsample
        self.inconv = nn.Conv2d(feature_dim * 2 + 1, feature_res, kernel_size=(3, 3), padding=1)
        self.outconv = nn.Conv2d(feature_res, 1, kernel_size=(3, 3), padding=1)
        self.layers = nn.ModuleList([ResBlockMotion(feature_res, expansion_ratio) for _ in range(num_res)])

    def forward(self, x: Tensor, feat_1: Tensor, feat_2: Tensor):
        _, _, height, width = feat_1.shape
        if self.upsample > 1:
            x = F.upsample_nearest(torch.unsqueeze(x, dim=1), (height, width))
        x = torch.cat([feat_1, x, feat_2], dim=1)
        x = self.inconv(x)
        for layer in self.layers:
            x = layer(x)
        x = self.outconv(x)
...
class ResBlockMotion(nn.Module):
    def __init__(self, n_feats: int, expansion_ratio: int, res_scale: int = 1.0):
        super(ResBlockMotion, self).__init__()
        self.res_scale = res_scale
        self.module = nn.Sequential(
            weight_norm(nn.Conv2d(n_feats, n_feats * expansion_ratio, kernel_size=(3, 3), padding=1)),
            nn.ReLU(inplace=True),
            weight_norm(nn.Conv2d(n_feats * expansion_ratio, n_feats, kernel_size=(3, 3), padding=1))
        )
