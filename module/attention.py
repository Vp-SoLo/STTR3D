...
class MultiheadAttention3D(nn.MultiheadAttention):
    def __init__(self, embed_dim, num_heads, batch=1):
        super(MultiheadAttention3D, self).__init__(embed_dim, num_heads, dropout=0.0, bias=True,
                                                   add_bias_kv=False, add_zero_attn=False,
                                                   kdim=None, vdim=None)
        self.n = batch

    def forward(self, query, key, value, attn_mask=None, pos_enc=None, pos_indexes=None):
        bhw, embed_dim = query.size()
        hw = int(bhw / self.n)
        head_dim = embed_dim // self.num_heads
        # to avoid the error
        q = None
        ...
        if torch.equal(query, key) and torch.equal(key, value):
            # self-attention
            q, k, v = F.linear(query, self.in_proj_weight, self.in_proj_bias).chunk(3, dim=-1)
            q = q.contiguous().view(self.n, self.num_heads, hw, head_dim)
            k = k.contiguous().view(self.n, self.num_heads, hw, head_dim)
            v = v.contiguous().view(self.n, self.num_heads, hw, head_dim)

        # cross-attention
        elif torch.equal(key, value):
            q = F.linear(query, self.in_proj_weight[:embed_dim][...], self.in_proj_bias[:embed_dim])
            k, v = F.linear(key, self.in_proj_weight[embed_dim:][...], self.in_proj_bias[embed_dim:]).chunk(2, dim=-1)
            q = q.contiguous().view(self.n, self.num_heads, hw, head_dim)
            k = k.contiguous().view(self.n, self.num_heads, hw, head_dim)
            v = v.contiguous().view(self.n, self.num_heads, hw, head_dim)

        raw_attention = torch.einsum('nhab,nhcd->nhac', q, k) * scaling
        # raw_attention = F.softmax((torch.einsum('nhab,nhcd->nhac', q, k) * scaling))

        raw_attn = raw_attention.contiguous().sum(dim=1).view(self.n, hw, hw)

        attn = torch.einsum('nhab,nhcd->nhad', F.softmax(raw_attention), v)

        attn = torch.cat([torch.squeeze(i, dim=1) for i in torch.split(attn, 1, dim=1)], dim=2)
        attn = attn.contiguous().view(bhw, self.embed_dim)
        return raw_attn, attn
