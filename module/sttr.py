...
from module.transformer_sttr import build_transformer
from utilities.integration_tools import batched_index_select, NestedTensor, Arguments
...
class STTR(nn.Module):
    ...
    def forward(self, x: NestedTensor):
        ...
        if x.sampled_cols is not None:
            feat_left_down = batched_index_select(feat_left, 3, x.sampled_cols)
            feat_right_down = batched_index_select(feat_right, 3, x.sampled_cols)
        if x.sampled_rows is not None:
            feat_left_down = batched_index_select(feat_left_down, 2, x.sampled_rows)
            feat_right_down = batched_index_select(feat_right_down, 2, x.sampled_rows)

        # transformer
        attn_weight = self.transformer(feat_left_down, feat_right_down, pos_enc)
...
def build_sttr(args: Arguments):
    return STTR(args=args)