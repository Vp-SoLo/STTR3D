...
def augment(input_data, transformation, args):
    """
...
def augment_disparity_change(input_data, transformation, args):
    if transformation is not None:
        # perform augmentation first
        input_data = transformation(**input_data)

    w = input_data['disp_t1'].shape[-1]
    # set large values to be 0
    input_data['disp_t1'][input_data['disp_t1'] > w] = 0
    input_data['disp_t2'][input_data['disp_t2'] > w] = 0
    input_data['disp_t1'][input_data['disp_t1'] < 0] = 0
    input_data['disp_t2'][input_data['disp_t2'] < 0] = 0
    # manually compute occ area (this is necessary after cropping)

    # to be honest, left disp in SceneFlow subset are all negative values, so we use right disp
    occ_mask = compute_left_occ_region(w, input_data['disp_t1'])
    input_data['occ_mask_t1'][occ_mask] = True  # update
    input_data['occ_mask_t1'] = np.ascontiguousarray(input_data['occ_mask_t1'])

    occ_mask = compute_left_occ_region(w, input_data['disp_t2'])
    input_data['occ_mask_t2'][occ_mask] = True  # update
    input_data['occ_mask_t2'] = np.ascontiguousarray(input_data['occ_mask_t2'])

    # manually compute occ area for right image
    try:
        occ_mask = compute_right_occ_region(w, input_data['disp_t1_right'])
        input_data['occ_mask_right_t1'][occ_mask] = 1
        input_data['occ_mask_right_t1'] = np.ascontiguousarray(input_data['occ_mask_right_t1'])
    except KeyError:
        # print('No disp mask right, check if dataset is KITTI')
        input_data['occ_mask_right_t1'] = np.zeros_like(occ_mask).astype(np.bool)
    input_data.pop('disp_t1_right', None)  # remove disp right after finish

    try:
        occ_mask = compute_right_occ_region(w, input_data['disp_t2_right'])
        input_data['occ_mask_right_t2'][occ_mask] = 1
        input_data['occ_mask_right_t2'] = np.ascontiguousarray(input_data['occ_mask_right_t2'])
    except KeyError:
        # print('No disp mask right, check if dataset is KITTI')
        input_data['occ_mask_right_t2'] = np.zeros_like(occ_mask).astype(np.bool)
    input_data.pop('disp_t2_right', None)  # remove disp right after finish

    # set occlusion area to 0
    occ_mask = input_data['occ_mask_t1']
    input_data['disp_t1'][occ_mask] = 0
    input_data['disp_t1'] = np.ascontiguousarray(input_data['disp_t1'], dtype=np.float32)
    occ_mask = input_data['occ_mask_t2']
    input_data['disp_t2'][occ_mask] = 0
    input_data['disp_t2'] = np.ascontiguousarray(input_data['disp_t2'], dtype=np.float32)
