...
def horizontal_flip_disparity_change(disp_left, disp_right, split):

    disp = disp_right[:, ::-1].copy()
    disp_right = disp_left[:, ::-1].copy()

    return disp, disp_right
...
def random_location(min_crop_height, min_crop_width, size, split):
    if split != 'train':
...
class UnifiedTransform(BasicTransform):
    """
    Transform applied to all data, without disparity change, and it's designed to process one frame data
    """
    ...
    def targets(self):
        return {
            "left": self.apply,
            "right": self.apply,
            "disp": self.apply_disp,
            "occ_mask": self.apply_mask,
            'occ_mask_right': self.apply_mask,
        }
...
class UnifiedTransformWithDispChange(BasicTransform):
    """
    Transform applied to all data, with disparity change
    """
    ...
    def targets(self):
        return {
            ...
            "right_t1": self.apply,
            "right_t2": self.apply,
            "disp_t1": self.apply_disp,
            "disp_t2": self.apply_disp,
            "occ_mask_t1": self.apply_mask,
            "occ_mask_t2": self.apply_mask,
            "occ_mask_right_t1": self.apply_mask,
            "occ_mask_right_t2": self.apply_mask,
            "disp_change": self.apply_disp
        }
    ...
    def update_params(self, params, **kwargs):
        ...
        if hasattr(self, "fill_value"):
            params["fill_value"] = self.fill_value
        params.update({"cols": kwargs["left_t1"].shape[1], "rows": kwargs["left_t1"].shape[0]})
        return params
...
class StereoTransform(BasicTransform):
    ...
    def update_params(self, params, **kwargs):
        ...
        try:
            ...
            params.update({"cols": kwargs["left_t1"].shape[1], "rows": kwargs["right_t1"].shape[0]})
        return params
...
class StereoTransformAsym(BasicTransform):
    ...
    def targets(self):
        return {
            "left": self.apply_l,
            "right": self.apply_r,
            "left_t1": self.apply_l,
            "right_t1": self.apply_r,
            "left_t2": self.apply_l,
            "right_t2": self.apply_r
        }
    ...
    #     return ["left", "right", "left_t1", "right_t1", "left_t2", "right_t2"]
...
class UnifiedMyCenterCrop(UnifiedTransform):
    """
    Center Crop
    """

    def __init__(self, args: Arguments, always_apply=1):
        super(UnifiedMyCenterCrop, self).__init__(always_apply)
        self.crop = CenterCrop((args.height, args.width))

    def apply(self, img, **params) -> np.ndarray:
        temp_type = img.dtype
        img = torch.permute(torch.Tensor(img), (2, 0, 1))
        img = self.crop(img)
        return torch.permute(img, (1, 2, 0)).numpy().astype(temp_type)

    def apply_disp(self, img, **params):
        temp_type = img.dtype
        img = self.crop(torch.Tensor(img))
        return img.numpy().astype(temp_type)

    def apply_mask(self, img, **params):
        img = self.crop(torch.Tensor(img))
        return img.numpy().astype(np.bool_)


class UnifiedMyCenterCropWithDispChange(UnifiedTransformWithDispChange):
    """
    Center Crop
    """

    def __init__(self, args: Arguments, always_apply=1):
        super(UnifiedMyCenterCropWithDispChange, self).__init__(always_apply)

        self.crop = CenterCrop((args.height, args.width))

    def apply(self, img, **params) -> np.ndarray:
        temp_type = img.dtype
        img = torch.permute(torch.Tensor(np.ascontiguousarray(img)), (2, 0, 1))
        img = self.crop(img)
        return torch.permute(img, (1, 2, 0)).numpy().astype(temp_type)

    def apply_disp(self, img, **params):
        temp_type = img.dtype
        img = self.crop(torch.Tensor(np.ascontiguousarray(img)))
        return img.numpy().astype(temp_type)
