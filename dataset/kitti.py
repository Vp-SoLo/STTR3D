...
from dataset.stereo_albumentation import UnifiedMyCenterCrop


class KITTIBaseDataset(data.Dataset):
    def __init__(self, datadir, args, split='train'):
    ...
    def _split_data(self):
        ...
        if len(self.left_data) > 1:
            if self.split == 'train':
                ...
                self.right_data = self.right_data
                self.disp_data = self.disp_data
    ...
    def _augmentation(self):
        if self.split == 'train':
            self.transformation = Compose([
                UnifiedMyCenterCrop(self.args),
                # it's hard to pre-train model with data argument, so I cut it off
            ...
            self.transformation = Compose([UnifiedMyCenterCrop(self.args)])
        else:
    ...
    def __getitem__(self, idx):
        ...
        left = np.array(Image.open(left_fname)).astype(np.float)
        input_data['left'] = left

        # right
        right_fname = self.right_data[idx]
        right = np.array(Image.open(right_fname)).astype(np.float)
        ...
        if not self.split == 'test':  # no disp for test files
            ...
            input_data = augment(input_data, self.transformation, self.args)
        else:
...
class KITTI2015Dataset(KITTIBaseDataset):
    def __init__(self, datadir,  args, split='train'):
        super(KITTI2015Dataset, self).__init__(datadir, args, split)
...
class KITTI2012Dataset(KITTIBaseDataset):
    def __init__(self, datadir,  args, split='train'):
        super(KITTI2012Dataset, self).__init__(datadir, args, split)
...
class KITTIDataset(KITTIBaseDataset):
    ...
    def __init__(self, datadir, args, split='train'):
        super(KITTIDataset, self).__init__(datadir, args, split)
